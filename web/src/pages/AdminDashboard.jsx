...
import { pointsFor, BONUS_POINT_OPTIONS } from "../lib/points";
...
export default function AdminDashboard() {
	...
	function choosePoints(activity, points) {
		setPendingPoints((prev) => {
			const current = activity.points || 0;
			const selected =
				prev[activity.id] !== undefined ? prev[activity.id] : current;
			const desired = selected === points ? 0 : points;
			const next = { ...prev };
			if (desired === current) delete next[activity.id];
	...
	async function handleSubmitPoints(ids) {
		const entries = ids
			.filter((id) => pendingPoints[id] !== undefined)
			.map((id) => [id, pendingPoints[id]]);
				...
				{pageTab === "users" && (
					<section className="glass rounded-2xl p-5 flex flex-col gap-4">
						...
						<div className="flex items-center justify-between border-t border-black/10 dark:border-white/10 pt-4">
							...
							>
								<ChevronDown
									className={`size-4 transition-transform ${usersExpanded ? "" : "-rotate-90"}`}
								/>
								Daftar pengguna
								<span className="text-xs font-normal text-neutral-400 dark:text-neutral-600">
									({users.length})
								</span>
							</button>
							{usersExpanded && (
					...
					<section className="glass rounded-2xl p-5 flex flex-col gap-4">
						...
						<div className="flex flex-wrap gap-2">
							<select
								...
								onChange={(e) =>
									updateFilter("user_id", e.target.value)
								}
						...
						<div className="overflow-x-auto scrollbar-thin -mx-5">
									...
									{report?.data.map((a) => (
										...
										>
											...
											<td className="px-5 py-3">
												...
												{!a.children?.length && (
													<p className="mt-1.5 text-xs italic text-neutral-400 dark:text-neutral-600">
														Mindful nutrition belum
														tercatat (data lama)
													</p>
												)}
											</td>
							...
							{report?.data.length === 0 && (
								<EmptyState
									icon={X}
									title="Tidak ada aktivitas yang cocok dengan filter ini"
								/>
							)}
						</div>
					</section>
...
function PointsToggle({ activity, pending, onChoose }) {
	const current = activity.points || 0;
	const value = pending !== undefined ? pending : current;
	const changed = pending !== undefined;

	if (activity.category === "bonus") {
		return (
			<div className="flex gap-1">
				{BONUS_POINT_OPTIONS.map((opt) => (
					<button
						key={opt}
						onClick={() => onChoose(activity, opt)}
						className={`rounded-lg px-2.5 py-1.5 text-xs font-medium transition whitespace-nowrap ring-1 ${
							value === opt
								? changed
	...
	return (
		<button
			onClick={() => onChoose(activity, given ? 0 : suggested)}
		...
		>
			...
			{given ? `${value} poin diberikan` : `Beri ${suggested} poin`}
		</button>
	);
}
